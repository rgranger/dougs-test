...
export class AppController {
  ...
  @Post('validation')
  validateMovements(@Res({ passthrough: true }) res: Response, @Body() body: MovementValidationBody) {
    if (!validateBody(body)) {
      res.status(HttpStatus.BAD_REQUEST)
      return { message: 'Invalid payload', reasons: validateBody.errors }
    }

    const result = this.appService.validateMovements(body.movements, body.balances);
