import { Balance, Movement } from "./types.ts";
...
export function validateMovements(
  ...
  return balances.reduce(
    (balancesResult, currentBalance, currentBalanceIndex) => {
      const previousBalance = currentBalanceIndex > 0
        ? balances[currentBalanceIndex - 1]
        : null;

      const movementsHash: Record<number, Movement> = {};
        ...
        if (movementsHash[currentMovement.id] === undefined) {
          ...
          if (
            (previousBalance === null ||
              previousBalance.date < currentMovement.date) &&
            currentMovement.date <= currentBalance.date
          ) {
            acc += currentMovement.amount;
          } else if (
            currentMovement.date > balances[balances.length - 1].date
          ) {
            // if movement date is after the last balance date
            balancesResult.type = "Error";
            balancesResult.reasons.push(
              "There are movements after the last balance Date.",
            );
          }
        }
      ...
      if (movementsTotal !== currentBalance.balance) {
        balancesResult.type = "Error";
        balancesResult.reasons.push(
          `Balance with Date "${currentBalance.date}" is not satisfied. Check if there are missing movements.`,
        );
      }
