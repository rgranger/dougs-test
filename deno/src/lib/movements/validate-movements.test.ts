...
import { validateMovements } from "./validate-movements.ts";
import { Movement } from "./types.ts";

Deno.test("url Deno.test", () => {
  const url = new URL("./foo.js", "https://deno.land/");
  assertEquals(url.href, "https://deno.land/foo.js");
});

Deno.test("it should return an error when no balance is provided", () => {
  assertEquals(
    validateMovements([], []),
    {
      type: "Error",
...
Deno.test("it should return an error when there movements after the last balance", () => {
  const movements = [
  ...
  assertEquals(
    validateMovements(movements, balances),
    {
      type: "Error",
...
Deno.test("it should return an error when there are insatisfied balances", () => {
  const movements = [
    { id: 12, date: new Date(1995, 5, 25).getTime(), amount: 100 },
    { id: 14, date: new Date(2023, 8, 21).getTime(), amount: -200 },
    { id: 22, date: new Date(1995, 3, 25).getTime(), amount: 50 },
  ...
  assertEquals(
    validateMovements(movements, balances),
    {
      ...
      reasons: [
        ...
        }" is not satisfied. Check if there are missing movements.`,
        `Balance with Date "${
          new Date(
            2024,
            8,
...
Deno.test("it should return a success when all the balances are satisfied (simple)", () => {
  const movements = [
...
Deno.test("it should return a success when all the balances are satisfied (multiple)", () => {
  const movements = [
    { id: 12, date: new Date(1995, 5, 25).getTime(), amount: 100 },
    { id: 14, date: new Date(2023, 8, 21).getTime(), amount: -200 },
    { id: 22, date: new Date(1995, 3, 25).getTime(), amount: 50 },
...
Deno.test("it should return a success when all the balances are satisfied (multiple with duplicates)", () => {
  const movements = [
...
Deno.test("it should return a success when there are no movements and the balances are 0", () => {
